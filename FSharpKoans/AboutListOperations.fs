    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
       let length (xs : 'a list) : int =
         let rec len xs count =
           match xs with
           | [] -> count
           | _::rest -> len rest (count + 1)
         len xs 0
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
           let rec len rest out = // write a function to reverse a list here.
            match rest with
            | [] -> out
            | x::rest -> len rest (x::out)
           len xs [] 
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
           let rec len lis out = // write a function which adds 1 to each element
            match lis with
            | [] -> List.rev(out)
            | x::lis -> len lis (x+1 ::out)
           len xs []  
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
           let rec len lis out = // write a function which doubles each element
            match lis with
            | [] -> List.rev(out)
            | x::lis -> len lis (x*2 ::out)
           len xs []   
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
           let rec len lis out = // write a map which applies f to each element
            match lis with
            | [] -> List.rev(out)
            | x::lis -> len lis (f x ::out)
           len xs []   
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
           let rec len lis out  = // write a function which filters based on the specified criteria
            match lis with
            | [] -> List.rev(out)
            | x::lis -> 
              match f x with 
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
           let rec len lis out  = // write a function to filter for odd elements only.
            match lis with
            | [] -> List.rev(out)
            | x::lis -> 
              match x%2=0 with 
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
           let rec len lis NS = // write a function to do what's described above
            match lis with
            | [] -> NS + initialState
            | x::lis -> len lis (x + NS)
           len xs 0  
    ...
    let ``16 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a = __// write a function to do a fold.
           (*let rec len lis NS = 
            match lis with
            | [] -> NS 
            | x::lis -> len lis (f x NS)
           len xs initialState  *)
        fold (+) 0 [1;2;3;4] |> should equal 10
    ...
    let ``18 exists: finding whether any matching item exists`` () =
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
           let rec len lis  = // Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
            match lis with
            | [] -> false
            | x::lis -> 
              match f x with 
    ...
    let ``19 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
           let rec len lis (out1,out2) = //Does this: https://msdn.microsoft.com/en-us/library/ee353782.aspx
            match lis with
    ...
    let ``21 tryFind: find the first matching element, if any`` () =
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
          let rec len lis  = // Does this: https://msdn.microsoft.com/en-us/library/ee353506.aspx
            match lis with
            | [] -> None
            | x::lis -> 
              match p x with 
    ...
    *)
    ...
    let ``23 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list = 
          (*let rec len lis out =  // Does this: https://msdn.microsoft.com/en-us/library/ee353456.aspx
            match lis with
            | [] -> None
            | x::lis -> 
              match p x with 
